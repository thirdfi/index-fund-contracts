...
describe("STI on BSC", async () => {
    ...
    before(async () => {
      ...
      strategyArtifact = await deployments.getArtifact("BscSTIStrategy");
      stVaultArtifact = await deployments.getArtifact("BscStBNBVault");
      nftArtifact = await deployments.getArtifact("StVaultNFT");
      priceOracleArtifact = await deployments.getArtifact("BscPriceOracle");
    });

    beforeEach(async () => {
      await deployments.fixture(["hardhat_bsc_sti"])
      ...
      stVault = new ethers.Contract(await strategy.BNBVault(), stVaultArtifact.abi, a1);
    ...
    describe('Basic', () => {
      ...
      it("Should be set with correct initial vaule", async () => {
        ...
        expect(await strategy.router()).equal('0x10ED43C718714eb63d5aA57B78B54704E256024E');
        expect(await strategy.SWAP_BASE_TOKEN()).equal(network_.Token.WBNB);
        ...
        expect(await stVault.name()).equal('STI Staking BNB');
        expect(await stVault.symbol()).equal('stiStBNB');
        ...
        expect(await stVault.stToken()).equal(network_.Token.aBNBb);
      });
      ...
      it("Should be set by only owner", async () => {
        ...
        await expect(vault.depositByAdmin(a1.address, [a2.address], [getUsdVaule('100')], 1)).to.be.reverted;
    ...
    describe('Basic function', () => {
      ...
      it("Basic Deposit/withdraw with small amount", async () => {
        ...
        expect(await stVault.pendingWithdrawals()).closeTo(BNBDeposits, BNBDeposits.div(100));
        expect(await vault.getAllPoolInUSD()).equal(0);
        ...
        expect(usdtBalance.add(waitingInUSD)).closeTo(getUsdtAmount('50000'), getUsdtAmount('50000').div(100));
