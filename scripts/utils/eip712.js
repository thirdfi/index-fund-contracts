...
function abiRawEncode(encTypes, encValues) {
  ...
  return getBufferFromHexStr(hexStr);
}
...
function getBufferFromHexStr(hexStr) {
...
function digestToSign(domain, primaryType, message, types = {}) {
  const v1 = domainSeparator(domain);
  const v2 = structHash(primaryType, message, types)
  return keccak256(
    Buffer.concat([
...
async function sign(domain, primaryType, message, types = {}, signer) {
  ...
  try {
    ...
    // const flatSig = await signer.signMessage(digest);
    // const recoveredAddress = ethers.utils.verifyMessage(digest, flatSig);
