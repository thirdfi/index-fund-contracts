...
module.exports = async ({ deployments }) => {
  ...
  // Deploy the StVault
  console.log("Now deploying EthStMATICVaultTest...");
  const proxy = await deploy("EthStMATICVaultTest", {
  ...
  console.log("  EthStMATICVaultTest_Proxy contract address: ", proxy.address);
  const STVault = await ethers.getContractFactory("EthStMATICVaultTest");
  ...
  const dataStVaultNft = nftIface.encodeFunctionData("initialize", [
    "STI Staking MATIC NFT", "stiStMATICNft",
    stVault.address,
  ]);
  ...
  try {
    ...
    await run("verify:verify", {
      address: implAddress,
      contract: "contracts/stVaults/lido/EthStMATICVaultTest.sol:EthStMATICVaultTest",
    });
  } catch (e) {
  }
  try {
...
module.exports.tags = ["ethRinkeby_sti_StMATIC"];
