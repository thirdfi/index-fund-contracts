...
module.exports = async ({ deployments }) => {
  ...
  // Deploy the StVault
  console.log("Now deploying BscStBNBVaultTest...");
  const proxy = await deploy("BscStBNBVaultTest", {
  ...
  console.log("  BscStBNBVaultTest_Proxy contract address: ", proxy.address);
  const STVault = await ethers.getContractFactory("BscStBNBVaultTest");
  ...
  const dataStVaultNft = nftIface.encodeFunctionData("initialize", [
    "STI Staking BNB NFT", "stiStBNBNft",
    stVault.address,
  ]);
  ...
  try {
    ...
    await run("verify:verify", {
      address: implAddress,
      contract: "contracts/stVaults/ankr/BscStBNBVaultTest.sol:BscStBNBVaultTest",
    });
  } catch (e) {
  }
  try {
...
module.exports.tags = ["bscTestnet_sti_StBNB"];
