...
module.exports = async ({ deployments }) => {
  ...
  const strategyProxy = await ethers.getContract("MWIStrategyTest_Proxy");
  const MWIStrategy = await ethers.getContractFactory("MWIStrategyTest");
  const strategy = MWIStrategy.attach(strategyProxy.address);

  console.log("Now deploying MWIVault...");
  const proxy = await deploy("MWIVaultTest", {
  ...
  try {
    ...
    await run("verify:verify", {
      address: implAddress,
      contract: "contracts/mwi/MWIVaultTest.sol:MWIVaultTest",
    });
  } catch (e) {
  }
};
module.exports.tags = ["avaxTestnet_mwi_MWIVault"];
