...
module.exports = async ({ deployments }) => {
  ...
  const stVaultProxy = await ethers.getContract("BscStBNBVault_Proxy");

  console.log("Now deploying BscSTIStrategy...");
  const proxy = await deploy("BscSTIStrategy", {
  ...
  console.log("  BscSTIStrategy_Proxy contract address: ", proxy.address);
  ...
  try {
    ...
    await run("verify:verify", {
      address: implAddress,
      contract: "contracts/sti/strategy/BscSTIStrategy.sol:BscSTIStrategy",
    });
  } catch (e) {
  }
};
module.exports.tags = ["bscMainnet_sti_STIStrategy"];
