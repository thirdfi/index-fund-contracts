...
module.exports = async ({ deployments }) => {
  ...
  const stVaultProxy = await ethers.getContract("AvaxStAVAXVault_Proxy");

  console.log("Now deploying AvaxSTIStrategy...");
  const proxy = await deploy("AvaxSTIStrategy", {
  ...
  console.log("  AvaxSTIStrategy_Proxy contract address: ", proxy.address);
  ...
  try {
    ...
    await run("verify:verify", {
      address: implAddress,
      contract: "contracts/sti/strategy/AvaxSTIStrategy.sol:AvaxSTIStrategy",
    });
  } catch (e) {
  }
};
module.exports.tags = ["avaxMainnet_sti_STIStrategy"];
