...
module.exports = async ({ deployments }) => {
  ...
  // Deploy the StVault
  console.log("Now deploying EthStMATICVault...");
  const proxy = await deploy("EthStMATICVault", {
  ...
  console.log("  EthStMATICVault_Proxy contract address: ", proxy.address);
  const STVault = await ethers.getContractFactory("EthStMATICVault");
  ...
  try {
    ...
    await run("verify:verify", {
      address: implAddress,
      contract: "contracts/stVaults/lido/EthStMATICVault.sol:EthStMATICVault",
    });
  } catch (e) {
  }
  try {
...
module.exports.tags = ["ethMainnet_sti_StMATIC"];
